...
videos_vec = {'Comb. Rides', 'Boxing'}; %, 'Comb. Explo SD', 'Bar SD', 'Underwater SD', 'Touvet SD'};
pref_mat = [preference_VW2,preference_VWBoxe]; %,preference_SD2,preference_SDBar,preference_SDUnderwater,preference_SDTouvet];
%{
alpha = 0.1; %0.05
figh=figure('Position', get(0, 'Screensize'));
...
resp_mat_ref = [responses_VW2_ref(:,question_ind),responses_VWBoxe_ref(:,question_ind)]; %,responses_SD2_ref(:,question_ind),responses_SDBar_ref(:,question_ind),responses_SDUnderwater_ref(:,question_ind),responses_SDTouvet_ref(:,question_ind)];
resp_mat_effect = [responses_VW2_effect(:,question_ind),responses_VWBoxe_effect(:,question_ind)]; %,responses_SD2_effect(:,question_ind),responses_SDBar_effect(:,question_ind),responses_SDUnderwater_effect(:,question_ind),responses_SDTouvet_effect(:,question_ind)];
%%{
figh=figure('Position', get(0, 'Screensize'));
set(figh, 'DefaultTextFontSize', 28);
mos_cut=2;
...
%% Plot fraction of users giving a responsiveness<=3 in ref and effect
question_ind=4; % considering mos on responsiveness
resp_mat_ref = [responses_VW2_ref(:,question_ind),responses_VWBoxe_ref(:,question_ind)]; %,responses_SD2_ref(:,question_ind),responses_SDBar_ref(:,question_ind),responses_SDUnderwater_ref(:,question_ind),responses_SDTouvet_ref(:,question_ind)];
resp_mat_effect = [responses_VW2_effect(:,question_ind),responses_VWBoxe_effect(:,question_ind)]; %,responses_SD2_effect(:,question_ind),responses_SDBar_effect(:,question_ind),responses_SDUnderwater_effect(:,question_ind),responses_SDTouvet_effect(:,question_ind)];
%%{
figh=figure('Position', get(0, 'Screensize'));
...
set(gca, 'FontSize',28)
hold off
saveas(figh,[foldername 'worstcase_resp_' filenames{question_ind}],'fig');
saveas(figh,[foldername 'worstcase_resp_' filenames{question_ind}],'epsc');
%}
...
%% Plot fraction of users prefering effect vs assessed resposiveness in effect
question_ind = 4;
resp_mat_ref = [responses_VW2_effect(:,question_ind),responses_VWBoxe_effect(:,question_ind)]; %,responses_SD2_ref(:,question_ind),responses_SDBar_ref(:,question_ind),responses_SDUnderwater_ref(:,question_ind),responses_SDTouvet_ref(:,question_ind)];
%{
figh=figure('Position', get(0, 'Screensize'));
set(figh, 'DefaultTextFontSize', 26);
end_mos=5;
...
set(gca, 'FontSize',24)
hold off
saveas(figh,[foldername 'prefvsResp_' filenames{question_ind}],'fig');
saveas(figh,[foldername 'prefvsResp_' filenames{question_ind}],'epsc');
%}